class TweetsController < ApplicationController
    ...
    before_action :move_to_index, except: [:index, :show, :select] #非ログインユーザーの書き込み画面侵入排除 ※クラスの決め事
  ...
  # def select
  #   @selects= Tweet.where(text:params[:text]).includes(:user).page(params[:page]).per(5).order("created_at DESC")
  #   # @selects= Tweets.where('text LIKE ?', '%1%').all
  #   # @selects= Tweet.select do |a|
  #   #   a= params[:text]
  #   # end
  # end
...
# text_ids = Tweet.where(text:params[:text]).pluck(:id)
# #キーワードで検索された名前と同じimageと同じimageを持つレコードのidを全て列挙する
# 
# @tweets_selected = Tweet.where("tweet_id IN (?) or tweet_id IN (?)", text_ids, text_ids)
# # text_idsかimage_idsに当てはまるものをtweetsテーブルから探して、@tweets_selected に代入。

# @tweets_selected = Tweet.where("tweet_id IN (?)", text_ids)
# #text_idsに当てはまるものをtweetsテーブルから探して、@tweets_selected に代入。
